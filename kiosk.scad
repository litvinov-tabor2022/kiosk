        ...
        translate([fatness, fatness, fatness - .01]) {
            ...
            translate([70.5, 1.4, - .01]) union() {
                ...
                difference() {
                    ...
                    cube(outter);
                    translate([f, f, b_fat]) cube([MFRC_board_size().x + inset, MFRC_board_size().y + inset, 100]);

                    translate([- .01, 18, 22]) cube([2, 15, 100]);
                    translate([- .01, 4, b_fat]) cube([100, outter.y - 2 * 4, 18]);
                    translate([10, - .01, b_fat]) cube([outter.x - 20, 100, 25]);
                ...
                // connections
                translate([19, f - .01, 25.5])  cube([f, MFRC_board_size().y + inset + .02, 2]);
                translate([41.9, f - .01, 25.5])  cube([f, MFRC_board_size().y + inset + .02, 2]);
            }

            translate(batt_holder_pos) difference() {
                BatteryHolder();
                translate([-.01, 15, 5]) cube([1000, 40, 10]);
            }
        }
    }
}
